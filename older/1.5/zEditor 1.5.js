...
var zeditor = {
	...
	quote: function (a) {
		...
		$('#ze-edit').load(a.href + ' #text_editor_textarea', function () {
			textarea.value = document.getElementById('ze-edit').firstChild.value.replace(/]/, '][link="' + location.pathname + '#' + a.href.match(/[0-9]+/) + '"]');
			textarea.focus();
			zeditor.loading('off')
		});
	},
	...
	button: function (where) {
		$(where).each(function () {
			$(this).find('a[href*="quote"]').attr('onclick', 'zeditor.start(\'quote\', this); return false').before('<a class="pbutton" href="#reply" onclick="zeditor.start(\'reply\', this)">' + lang.reply_button + '</a>');
	...
	newPost: function (a) {
		...
		if (editor_mode.innerHTML == lang.reply || editor_mode.innerHTML == lang.quote) {
			$('<div class="zeditor-new"></div>').insertAfter(editor_post + ':last').load(a + " #p" + b, function () {
				zeditor.button('.zeditor-new' + editor_options + ':last');
	...
	replace: function (a) {
		return a.replace(/\[tag\](.*?)\[\/tag\]/g, function (a, b) {
			...
			return ' ' + lang.quote_message + ' <a href="' + b + '"> ' + b.split("#")[1] + '</a></cite>'
		})
	},
	loading: function (a) {
