var zeditor = {
	...
	start: function (a, dom) {
		...
		zeditor.editor.style.display = 'block';
		switch (a) {
		case "reply":
			zeditor.url = $('a[href^="/post?t="]').first().attr("href");
	...
	newPost: function (a) {
		...
		if (zeditor.mode.innerHTML == zeditor.lang.reply || zeditor.mode.innerHTML == zeditor.lang.quote) {
			$.get(a, function (data) {
				$('<div class="zeditor-new">' + zeditor.replace($(data).find("#p" + b).html()) + '</div>').insertAfter(zeditor.post_dom + ':last');
				zeditor.button('.zeditor-new ' + zeditor.button_dom + ':last');
				$('html,body').animate({
	...
	copyright: function () {
		if (confirm("zEditor 1.6 by Zero. Click OK to visit my thread for updates\nzEditor 1.6 b?i Zero. B?m OK ð? xem thêm thông tin")) {
